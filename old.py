...
def main():
    ...
    print('Load network....')
    net.load_state_dict(torch.load('./weights/ERES.pth'))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if os.path.exists('./weights/20eRes{}.pth'.format(epoch-1)):
            net = build_model('train', cfg.NUM_CLASSES, width_mult=0.0625) 
            net.load_state_dict(torch.load('./weights/20eRes{}.pth'.format(epoch-1)))
        ...
        if epoch % args.epoch_prune == 0 or epoch == args.epochs - 1:  #here it prunes
            ...
            torch.save(net.state_dict(), './weights/20eRes{}.pth'.format(epoch))
