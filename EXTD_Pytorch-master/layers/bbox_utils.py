...
def match(threshold, truths, priors, variances, labels, loc_t, conf_t, idx):
    ...
    N = torch.div(torch.sum(best_prior_overlap >= _th2) +
              torch.sum(best_prior_overlap >= _th3), 2, rounding_mode='floor')

    matches = truths[best_truth_idx]          # Shape: [num_priors,4]
    conf = labels[best_truth_idx]         # Shape: [num_priors]
    #conf[best_truth_overlap < _th2] = 0  # label as background
    conf[best_truth_overlap < _th2] = torch.zeros_like(conf[best_truth_overlap < _th2], dtype=conf.dtype, device=conf.device)
...
def nms(boxes, scores, overlap=0.5, top_k=200):
    ...
    # xx1 = boxes.new().resize_(0)
    # xx1 = torch.empty(dtype = torch.float32)
    # yy1 = boxes.new().resize_(0)
    # xx2 = boxes.new().resize_(0)
    # yy2 = boxes.new().resize_(0)
    w = boxes.new()
    h = boxes.new()
