...
def get_data():
    ...
    if subset == 'val':
        wider_face = sio.loadmat(
            './eval_tools/wider_ground_truth/wider_face_val.mat')
    else:
        wider_face = sio.loadmat(
...
def eval_wider(model_name):
    ...
    net.load_state_dict(torch.load(model_name), strict=True)  #was strict=True, but I changed it to False to load the model with masks in buffer
    net.eval()
    with torch.no_grad():
        if use_cuda:
            net.cuda()
