...
parser = argparse.ArgumentParser(description='Pruning with L1NormPruner',
...
parser.add_argument('--pruned_eres', type=str, default='./weights/n40eRes', help='path to save pruned weights without epoch number')
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        model_path = args.pruned_eres + '{}.pth'.format(epoch - 1)
        if os.path.exists(model_path):
            net = build_model('train', cfg.NUM_CLASSES, width_mult=0.0625)
            net.load_state_dict(torch.load(model_path))
            optimizer = optim.Adam(net.parameters(), lr=current_learning_rate, weight_decay=args.decay)
        ...
        if epoch % args.epoch_prune == 0 or epoch == args.epochs - 1:
            ...
            torch.save(net.state_dict(), args.pruned_eres + '{}.pth'.format(epoch))
